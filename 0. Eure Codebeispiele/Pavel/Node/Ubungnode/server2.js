...
function checkUsPass(req, res) {
  const { username, password } = req.body;

  if (!username || !password) {
    return res.status(400).send("use username and passowrd");
  }
...
app.get("/filter", (req, res) => {
  ...
  res.json(jokes.filter((joke) => joke.jokeType === jokeType));
});

app.delete("jokes/:id", (req, res) =>{
const jokeId= parseInt(req.params.id);
const deletedJoke = jokes.find((element) => (element.id === jokeId));
jokes.splice(jokes.indexOf(deletedJoke),1);
})
...
app.get("/site", (req, res) => {
  res.redirect("https://www.codersbay.at");
});

app.post("/login", checkUsPass);
